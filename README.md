...
```shell
  docker buildx build -t psdj/test-extension:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install psdj/test-extension:latest
```
...
```shell
  docker extension dev ui-source psdj/test-extension:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug psdj/test-extension:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset psdj/test-extension:latest
```
...
```shell
docker extension update psdj/test-extension:latest
```
...
```shell
docker extension rm psdj/test-extension:latest
```
